...
def save_request(request_args):
    ...
    request_args = [['--'+key, value] for key,value in request_args.items() if value is not None]
    request_args_to_plain_text = str(request_args).replace('[','').replace(']', '').replace(':', '').replace('\'', '').replace(',','')
